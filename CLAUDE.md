...
| Server OS info: read on every connect (os-release, uname, macOS, Windows) and saved on the host; the host list shows it with a distro/OS icon | Coordinator | db1c6b0, ba6d9c2, d80018f, 7260967 | UAT failed (third try): "tulisan ubuntunya di remove aja kan udah ada iconnya itu sudah cukup sih" and "terus tulisan versionnya di kecilkan lagi", so only the version goes under the logo, in smaller text; fix in development (earlier: "yang server info ini ngebuat card sizenya tidak konsisten", uneven cards; then "mending tulisan Ubuntu 22.04.5 LTS dipindah di bawah logonya saja … arch version di takeout saja") |
...
| Port forwards on the tablet (`ssh -L`): rules saved per host open 127.0.0.1:<port> on the tablet when a session connects, e.g. for a DB app | Coordinator | d8b0fe1, 9d719a0 | UAT failed: "untuk portforward ini buatnya headless session, jadi perlu ada screen tersendiri dimana user bisa pilih existing atau create new host lalu masukkan port yang mau dibuka dan mappingnya ke port apa, dan 1 setting bisa open beberapa port sekaligus"; redesign (own screen, headless, several ports per setting) in development |
| Theme in Settings: light/dark/system mode and a colour palette for the whole app, terminal included | Coordinator | f3ad854, 73026d6 | on tablet, UAT pending (after a first failed try: "theme and collor pallet yang aku maksud bukan cuma applikasi tapi di color schema terminalnya juga harusnya berubah juga", the terminal kept its own dark colours) |
