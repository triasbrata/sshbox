...
| Toasts in the toastification style at the top, auto-closing (1s by default); the port-forward notice becomes a 5s toast with Open | Coordinator | 6764896, 89f37e0 | UAT failed: the port-forward toast did not appear. The toast works in tests; the server had started before the session connected, so nothing was forwarded. Forward failures are now red toasts (89f37e0), on main, waiting for an app relaunch |
...
| Holding a selection handle at the top or bottom edge keeps the terminal scrolling | Coordinator | 40bcdd9, 7df6997, 479c2a5, 46e9fcd | Dropped by the user after three failed UATs; removed in 46e9fcd (on main, waiting for the app relaunch) |
