...
|---|---|---|---|
| Database browser for PostgreSQL, MongoDB and Redis, from Home: each saved database is reached through a saved host's SSH connection (like `ssh -L`, jump hosts too), lists its tables, collections or keys, and runs SQL, database commands or Redis commands with the result in a grid | database client ui development | c06f951 | in development: on branch worktree-db-browser with main (100f84f) merged in, analyze and tests clean, checked against real PostgreSQL 18, MongoDB 7 and Redis servers on this machine, and the coordinator's review passed; waiting for main to be fast-forwarded, then for the next APK install, since main has native code now and a hot restart would be lost (asked for in "develop rdmbs ui for mongodb, redis, and postgress") |
| Share with Jeansh goes to the session of the Jeansh already running instead of starting a second copy: the SEND filters move from MainActivity to a new ShareActivity (no window, never in Recents), which forwards the share to Jeansh's own task (NEW_TASK finds it by MainActivity; CLEAR_TOP with SINGLE_TOP hands it to onNewIntent, closing a file picker or Custom Tab left open over it) and finishes; with Jeansh not running it still cold-starts through `pending`; MainActivity stays singleTop, because singleTask would close a picker or Custom Tab on every launcher tap and still make a copy for a sender that asks for a result; and a launcher tap that Android answers with a second MainActivity on top of Jeansh's (a file picker or Custom Tab open over it, after a share or a notification tap that cold-started the app) finishes that copy in onCreate before it runs any Dart, so the task shows as it was | Coordinator | 65d4255, 7481f81 | in development (asked for in "tadi ketika share with jeansh, applikasi jeansh malah spawn applikasi baru bukannya upload ke existing session di applikasi yang sedang berjalan. berikut videonya /tmp/Screenrecorder-2026-09-15-00-41-34-998.mp4") |
