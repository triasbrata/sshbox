...
| tmux found outside a non-login PATH: the app looks for tmux on PATH, then in Homebrew (/opt/homebrew/bin, /usr/local/bin), MacPorts, Linuxbrew, Nix, ~/.local/bin and snap, then in the login shell's PATH, and runs every tmux by that full path | Coordinator | f79322d | on tablet by hot reload, UAT pending ("ini untuk deteksi tmux sepertinya ngebug kalau aku coba di macos yang install tmux di /opt/homebrew/bin/tmux di anggap tidak ada tmux di servernya") |
