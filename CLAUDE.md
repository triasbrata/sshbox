...
|---|---|---|---|
| macOS app and an Apple build pipeline: `tools/build_apple.sh` builds on a Mac the macOS app (.zip and .dmg, with the app icon), an iOS simulator build and an iOS device .ipa (unsigned, or signed with `--team`); secrets on macOS go in the login keychain | apple build pipeline | — | in development ("aku butuh kamu untuk build script pipeline yang bisa di jalankan di macos untuk build versi macos dan ios") |
