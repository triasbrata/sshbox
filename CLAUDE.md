...
| Port forwarding (`ssh -L`) on its own screen from Home: a setting picks a saved or new host and opens several 127.0.0.1 ports on the tablet over a headless SSH connection, switched on and off, e.g. for a DB app | Coordinator | d8b0fe1, 9d719a0, 158e1f8 | UAT failed (second try): "ini aku nga ngerti cara pakainya, ux nya mending di buat pilih arah port forward apakah dari tablet ke remote, atau remote ke table, lalu port yang mau di forward  dan ada pilihan advance untuk mengisi mengcustom port atau hostnya" and "user juga di kasikan port snippet, misalkan dia mau connect untuk postgrest"; editor rework (direction, one port, Advanced, Remote → Tablet, common-port snippets) in development (first try: "untuk portforward ini buatnya headless session, jadi perlu ada screen tersendiri …") |
| Theme in Settings: light/dark/system mode and named colour themes (Clode, Dracula, Nord, Gruvbox, Solarized, Catppuccin, Tokyo Night, One Dark) for the app and the terminal | Coordinator | f3ad854, 73026d6, 627d7a1 | on tablet, UAT pending (after two failed tries: "theme and collor pallet yang aku maksud bukan cuma applikasi tapi di color schema terminalnya juga harusnya berubah juga", then "untuk theme ini kayanya sama aja", where switching palettes changed only a faint terminal tint and the cursor) |
