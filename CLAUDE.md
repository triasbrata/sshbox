...
|---|---|---|---|
| Custom key picker: Clear resets the picked key and modifiers; a fourth modifier, Super/Cmd (CSI modifier 8 on special keys, CSI u on the rest); a PC or macOS layout (⌃ ⌥ ⇧ ⌘, and on macOS ⌘←/→, ⌥←/→, ⌘⌫ and ⌥⌫ edit text like iTerm2's Natural Text Editing), remembered and saved with each key | Coordinator | — | in development ("add keybar sudah sesuai dengan yang aku mau cum tinggal tambahkan reset key yang user pilih, sama ini kurang key super/cmd di mac. ini perlu di tambahkan opsi layout juga untuk layout macos") |
