...
|---|---|---|---|
| Key bar settings: Settings → Keyboard → Key bar lists every on-screen key and divider with a show/hide switch and drag to reorder, a live preview and Reset to default; the files and upload icons stay fixed at the front | Coordinator | — | in development ("need add the keyboard on screen settings where user can manage this section"; a first attempt was stopped, then "key bar settings continue") |
| Every message shows as the new toast, with no old grey snackbars left, and toasts are plain light or dark following the terminal theme instead of coloured by type | Coordinator | — | in development ("toast nya ini kenapa tidak pakai toast engine yang baru?", then "toastnya tidak usah berwana warni, cukup light and dark saja sesuai dengan theme dari terminalnya") |
