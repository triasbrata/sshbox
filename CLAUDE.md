...
|---|---|---|---|
| Signed relay notifications, SNAP-style: the app makes one ECDSA P-256 key per host, registers only the public key with the jeansh-notify relay, and passes the private key to that host as LC_SSHBOX_KEY; servers sign each send (X-PARTNER-ID, X-TIMESTAMP, X-EXTERNAL-ID, X-SIGNATURE over method, path, body hash, timestamp and external id), and the relay checks the signature, the time window and replays, and takes the host from the key; deleting a host revokes its key, and the bearer relay key and LC_SSHBOX_TOKEN go away | Coordinator | — | in development ("kita perlu ada mekanisme ketika ngirim notifikasi ke device user via relay cf worker, user harus pegang key dan payload itu harus di sign pakai key tersebut. dan key itu langsung di passing ke variable key ketika connect ke session tersebut. dan bodynya juga harus ada body validate signature  mirip seperti protokol SNAP di payment gateway. jadi device tablet register key yang valid, nanti server pakai key yang valid itu untuk memvalidasi payload dan server pengirimnya") |
