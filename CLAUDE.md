...
|---|---|---|---|
| Key bar custom keys: Add key offers the built-in keys not in the bar, a divider and Custom key… (a label and the text it sends, with \n, \t, \e and \xHH escapes, editable later); each row's show/hide switch becomes a remove button | Coordinator | — | in development ("key bar settings pass, but user cant add more custom keys, need user can add custom keys, and replace the toggle to remove") |
