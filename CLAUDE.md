...
| Theme in Settings: light/dark/system mode and named colour themes (Clode, Dracula, Nord, Gruvbox, Solarized, Catppuccin, Tokyo Night, One Dark) for the app and the terminal | Coordinator | f3ad854, 73026d6 | UAT failed (second try): "untuk theme ini kayanya sama aja" (switching palettes changed only a faint terminal tint and the cursor); the user chose named schemes, in development (first try: "theme and collor pallet yang aku maksud bukan cuma applikasi tapi di color schema terminalnya juga harusnya berubah juga") |
