...
|---|---|---|---|
| Settings page (⚙ on the host list) with a terminal font picker (Cascadia Mono, Cascadia Code, CaskaydiaCove Nerd Font, JetBrains Mono, Fira Code) and font size, live everywhere | Coordinator | 09ced89 | on tablet, UAT pending |
| App renamed to Clode (visible name only; package id, settings keys, tmux names and the sshbox:// scheme keep "sshbox") | Coordinator | 66d22e3 | on tablet, UAT pending |
| The Tailscale sign-in link opens in a web tab too (Google may refuse embedded sign-in; Open in browser is the fallback) | Coordinator | bf149c7 | on tablet, UAT pending |
...
| Holding a selection handle at the top or bottom edge keeps the terminal scrolling | Coordinator | 40bcdd9, 7df6997, 479c2a5 | on tablet, UAT pending (second retest; the last try scrolled one line, likely the end of that tab's scrollback, and the top zone reached into the status bar, now fixed by keeping the zone inside the terminal) |
