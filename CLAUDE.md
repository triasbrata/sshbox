...
| Port forwarding (`ssh -L`) on its own screen from Home: a setting picks a saved or new host and opens several 127.0.0.1 ports on the tablet over a headless SSH connection, switched on and off, e.g. for a DB app | Coordinator | d8b0fe1, 9d719a0, 158e1f8 | on tablet, UAT pending (after a first failed try: "untuk portforward ini buatnya headless session, jadi perlu ada screen tersendiri dimana user bisa pilih existing atau create new host lalu masukkan port yang mau dibuka dan mappingnya ke port apa, dan 1 setting bisa open beberapa port sekaligus") |
