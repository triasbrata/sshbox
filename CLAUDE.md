...
|---|---|---|---|
| Tailscale sign-in from the connect sheet opens in a web tab again: Open link closes the sheet, the session gets its tab with the sign-in page beside it, and the page closes itself once signed in | Coordinator | 1ed6757 | on tablet, UAT pending (after "login link tailscale belum open browser in tab"; the connect sheet had sent it to the phone's browser) |
| Push token passed to each host on connect as LC_SSHBOX_TOKEN and LC_SSHBOX_HOST_ID (tmux too) and read by sshbox-notify; the Home key button leaves, and Settings keeps "Copy notification token" as a fallback | Coordinator | 9f6df5b | on tablet, UAT pending: the variables reach the host ("echo $LC_SSHBOX_TOKEN $LC_SSHBOX_HOST_ID ini pass"); the Home key button leaving and Settings' "Copy notification token" are not checked yet |
