...
|---|---|---|---|
| Push token passed to each host on connect as LC_SSHBOX_TOKEN and LC_SSHBOX_HOST_ID (tmux too) and read by sshbox-notify; the Home key button leaves, and Settings keeps "Copy notification token" as a fallback | Coordinator | 9f6df5b | on tablet, UAT pending |
| App renamed to Jeansh with the tagline "Terminal buddy in your pocket" under the Home title, and a new launcher icon from the user's denim-pocket-and-terminal picture, legacy and adaptive | Coordinator | 64e873b, 417fb09, 414d352 | on tablet, UAT pending: the launcher name and the Home tagline passed ("oke pass point 1 dan 2"); the icon was swapped for the user's second picture ("ganti iconnya pakai ini /tmp/Rectangle_18__1_.png") and is now installed; the new icon, the Settings theme name, the keep-alive notification and the share list are not checked yet |
