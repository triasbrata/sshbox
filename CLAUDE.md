...
|---|---|---|---|
| Known hosts in Settings: every trusted host fingerprint listed (address, the saved hosts using it, fingerprint), and Forget drops one so the next connection asks again | Coordinator | a9ea390 | on tablet, UAT pending |
...
| Host editor: Choose file reads a private key into its field (OpenSSH/PEM, 64 KB max; a public key or .ppk is refused with a hint); passphrase and password get a show/hide eye | Coordinator | 80a145e, c4fc025 | on tablet, UAT pending |
