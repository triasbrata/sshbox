...
|---|---|---|---|
| Database browser for PostgreSQL, MongoDB and Redis, from Home: each saved database is reached through a saved host's SSH connection (like `ssh -L`, jump hosts too), lists its tables, collections or keys, and runs SQL, database commands or Redis commands with the result in a grid | database client ui development | c06f951 | in development: on branch worktree-db-browser, analyze and tests clean, and checked against real PostgreSQL 18, MongoDB 7 and Redis servers on this machine; waiting for the coordinator to fast-forward main and put it on the tablet (asked for in "develop rdmbs ui for mongodb, redis, and postgress") |
