...
| Share with Jeansh goes to the session of the Jeansh already running instead of starting a second copy: the SEND filters move from MainActivity to a new ShareActivity (no window, never in Recents), which forwards the share to Jeansh's own task (NEW_TASK finds it by MainActivity; CLEAR_TOP with SINGLE_TOP hands it to onNewIntent, closing a file picker or Custom Tab left open over it) and finishes; with Jeansh not running it still cold-starts through `pending`; MainActivity stays singleTop, because singleTask would close a picker or Custom Tab on every launcher tap and still make a copy for a sender that asks for a result | Coordinator | 65d4255 | in development (asked for in "tadi ketika share with jeansh, applikasi jeansh malah spawn applikasi baru bukannya upload ke existing session di applikasi yang sedang berjalan. berikut videonya /tmp/Screenrecorder-2026-09-15-00-41-34-998.mp4") |
